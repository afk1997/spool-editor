...
const ACTION_ERROR_COPY: Readonly<Record<string, string>> = {
  ...
  egress_consent_required: "Allow network access for this action, then try again.",
...
function isTransportFailure(error: unknown): boolean {
  return error instanceof TypeError && /(?:failed to fetch|fetch failed|network|load failed)/i.test(error.message);
}
...
export function describeActionError(error: unknown, fallback?: string): ActionErrorDescription {
  ...
  const message = ACTION_ERROR_COPY[code]
    ?? structured.message
    ?? fallback
...
export function actionError(error: unknown, fallback?: string): ActionErrorDescription & { rawCode: string } {
