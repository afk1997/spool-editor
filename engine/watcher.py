...
def reconcile_watch(watch: dict, *, list_items, ingest, transcript_done, produce) -> dict:
    """Advance ``watch`` one tick. Returns the new ``seen``/``pending``/``produced`` state plus the
    ``ingested`` source ids and ``produced_now`` source ids from THIS tick (for persistence +
    reporting). Disabled watches are a no-op. ``pending`` maps an item key → its source id."""
    if not watch.get("enabled", True):
        return {"seen": watch.get("seen", []), "pending": watch.get("pending", {}),
                "produced": watch.get("produced", []), "ingested": [], "produced_now": []}
    ...
    # 2) produce for pending sources whose transcript is now done
    ...
    return {"seen": seen, "pending": pending, "produced": produced,
