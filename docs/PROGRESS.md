...
>
> ---
> **Session 2026-06-12 — full audit-fix pass: every high+medium from `docs/CODE_REVIEW.md`. ✅ done, merged to main (not pushed).**
> Engine **854 tests** (was 807) · studio typecheck/lint/**37 vitest**/build green · **40 commits** (`41a7ad0..0ef2c24`), one per fix, each implemented TDD and passed a spec review + a code-quality review (subagent-driven; six review rounds forced real fix-ups before approval).
>
> - **✅ All 7 HIGHs:** download timeout 300s→3600s + `TROVE_DOWNLOAD_TIMEOUT` (`5597a7e`) · roi_motion derives the timeline from real `pts_time`, not `/30` — non-30fps reframe fixed, plus empty/mismatch guards (`2299fc3`) · transcript virtualization switched to an element virtualizer on `.main` — long transcripts reachable again (`4ace3bf`) · clean SSE EOF now reconnects instead of freezing behind a green badge (`345b106`) · queued downloads can't be resurrected after cancel/pause (`6fa5228`) · MCP falls back to stdio on engine-down `SystemExit` (`e51927b`) · watch-target argv-injection RCE closed with `--` + `is_safe_url` at both layers (`110c9dd`).
> - **✅ All 24 MEDIUMs.** Highlights: persist-under-lock + unique tmp + logged failures — and the review loop caught a deeper bug the audit missed: `asdict()` over a live Popen raised `TypeError` and silently dropped CANCELLED persists; both managers now build payloads from a `getattr` allowlist (`ebbfc4d`+`4ce38e7`+`0d51c76`) · WatchStore RLock (`e623d86`) · wall-clock `started_at` (`b066479`) · cross-watch ingest dedup (`cf7b062`) · folder mtime settle gate (`c8998df`) · playlist-listing TTL+backoff with manual-scan invalidation (`7c55b1b`) · face-track `budget==0` ZeroDivision (`c95208d`+`6454c56`) · single-decode L/R ROI motion via `split=2` — output byte-identical, half the decode (`cbf7741`) · `-pix_fmt yuv420p` on export + intermediates (10-bit/4:2:2 sources now play) (`d9d0187`) · ASS word-text escaping + font sanitize, incl. a validator that sanitized-but-stored-raw (`73ef0be`) · `flat_text` walks segment order — `insert_after` no longer corrupts search/FTS (`29ac718`) · **agent confirmation gate**: exports + delete/settings/model tools return `action:"confirm"` until the turn carries `confirm_tool`; studio Confirm/Cancel card wired end-to-end, name-only single-use semantics documented (`1f0944a`+`a3a6dcf`+`d5aee4e`) · agent retries transient LLM failures and finishes with partial results (`e7d35b0`) · atomic `stream_to` (`da04485`) · diarizer decodes the WAV once (`a46a830`) · bulk caps at 50 + titles resolve on the worker thread (`dd6e92c`) · `is_safe_url` fails CLOSED on DNS failure + honest TOCTOU docstring; api_v1 fixtures pinned to IP literals for offline determinism (`56df2d1`+`54678c2`) · **Offline mode is real**: persisted `offline` setting drives `SPOOL_OFFLINE`, enforcement proven by an in-process probe — egress provider actually raises (`15dfe20`) · editor inspector follows live clip updates (`1770fa3`) · mutation chains await submits + surface failures (`ea65865`) · SpoolProvider memoized — SSE deltas stop churning every consumer (`2a7d7d2`) · api-client 30s timeout + AbortSignal, agent 600s (`cd02212`) · studio tsconfig extends the base — 38 strictness errors fixed across 11 files (`cbf4605`) · CI runs vitest (`0ef2c24`) · yt-dlp pinned to the actual latest release **2026.6.9** + pytest-timeout active — a first pin to a stale-index 2025 release was caught in review and corrected (`fbc1b43`+`c18ef5d`).
> - **Carried-forward (pre-existing, confirmed real, deliberately untouched):** test_safety `_make_app` sets `TROVE_TOKEN` without cleanup (order-dependent 401 cascade if test_safety collects before test_api_v1 in one process) · `test_is_safe_url_accepts_public` does live DNS (youtube/vimeo/tiktok) · engine-context backoff resets on every snapshot (a pathological instant-close loop pins ~1Hz reconnect) · `_measure_roi_motion` now has zero production callers (kept as the documented per-ROI primitive) · persist stale-overwrite window (two full snapshots, last-writer-wins — accepted, matches the clip_jobs reference pattern) · `produce_clips`/`scan_watch` transitively reach the export stage un-gated (documented residual in `agent_tools.py`).
> - **Out of scope by decision:** the audit's 133 lows (incl. Docker, medium→low) and Playwright-in-CI (needs a live-engine fixture design first — deliberately not added as flaky CI noise).
> - Execution plan + per-task TDD detail: `docs/superpowers/plans/2026-06-12-code-review-high-medium-fixes.md` (untracked). The audit itself: `docs/CODE_REVIEW.md` (untracked).
...
- [x] **Studio + packages** — real Next.js 16 app; `@spool/types` (full data model, spec §3), `api-client`, and `ui`; home screen wired to the engine. Agent integrations use the working Python FastMCP stdio server in `engine/mcp_server.py`, not a TypeScript MCP package. *(fix `d788686`: health() shape)*
...
**Phase 0 done-when:** from a clean checkout, the engine runs headless → POST a URL to `api_v1` → file downloads with live progress → transcribe yields `words.json` + `.srt`; Claude Desktop can inspect the same state through the Python FastMCP server, while MCP mutations return `agent_mutation_disabled` and make zero underlying calls; no htmx anywhere.
...
- [x] **MCP clip tools** + elicitation + `spool://` resources — extended trove's FastMCP
  ...
  round-trip) + parity test. The Phase 0 safety fuse now keeps reads/resources live but returns
  `agent_mutation_disabled` before every mutating client call; execution resumes only after the
  Phase 4 approval and undo contract ships.
